...
> **收斂結論**：Noki 的「可自定風格」= **可選風格原型（版型的本質）** + **品牌識別覆蓋（租戶自填）** + **明暗模式（使用偏好）**。三者正交組合，用同一套 token 疊出最終主題。這同時延續產品設計原則 #1（UI 與 Agent 共用動作層）的精神：**風格也是一套有型別的 token，不是散落各處的硬編碼色碼。**
...
> 每個原型都對應 Noki 研究中真實存在的教練類型，並同時提供 Light / Dark 兩版值。

> **以下風格模擬以 Pencil 實際繪製**：同一個 Noki 學員端卡片元件（品牌標記／今日課表／排課按鈕／統計卡／學員列），套上不同主題即換膚 —— 印證「一份元件 × N 種主題」的可行性。原始檔：`design/noki-styles.pen`（token 已存為主題化變數）。

![Noki 6 風格原型 × Dark/Light 全覽](screenshots/style-showcase.png)

### 2.1 Pulse 活力（平台預設）
- **定位 / 人格**：現代、親和、乾淨。Noki 的中性預設，誰都不出錯。
...
### 對照表：Noki 原型 ↔ OneFloors 版型

| Noki 原型 | 對應 OneFloors 版型 | 教練類型 |
|----------|-------------------|---------|
| Pulse 活力 | （Noki 新增，平台預設） | 綜合 / 一般私教 |
...
| 自訂網域 / Slug | `noki.cc/coach/<名>` → 進階自訂網域 | slug 唯一；網域 = Phase 3 | ◐→Phase3 |
| 通知寄件識別 | Email / LINE 署名、頁首 | 文字 + Logo | Phase 1.5 |
| 學員端品牌開關 | 學員看到的是工作室品牌還是 Noki | 布林 | Phase 2 |
| 預設明暗偏好 | 該租戶面向客戶頁的預設明暗 | Light / Dark / 跟隨 | ◐ |
| 自訂 CSS / 完整白標 | 移除 Noki 標記、注入樣式 | 進階方案限定 | Phase 3 |
...
> **風格已存檔**：六原型 × 明暗的完整 token 矩陣已存為 Pencil 主題化變數於 `design/noki-styles.pen`（兩條主題軸 `archetype` × `mode`，§2 各表的值即由此匯出）。此檔是設計端的單一事實來源，可直接匯出成前端 CSS variables 或 design token JSON。
...
| **Phase 3** | 自訂網域、自訂 CSS、移除 Noki 標記的**完整白標**（高階方案）。 |
...
3. **學員端品牌歸屬**：學員看到「工作室品牌」會稀釋 Noki 自身曝光（獲客 vs. 白標訴求的取捨）→ 預設給誰？是否做成方案差異（免費層露 Noki、付費層可隱藏）？
